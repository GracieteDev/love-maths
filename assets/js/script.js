...
// Get the button elements and the add event listeners to them 
    ...
    for (let button of buttons) {
        button.addEventListener("click", function () {
            if (this.getAttribute("data-type") === "submit") {
                checkAnswer();                                     // delete alert("You clicked Submit!");
            } else {
...
function incrementScore() {
    ...
    document.getElementById("score").innerText = ++oldScore;               //problem if used  oldScore+1; the result would not update as it would be returned to the DOm befor adding 1;
...
function displayDivisionQuestion(operand1, operand2) {
    ...
    // Ensure operand2 is not 0 to avoid division by zero
    if (operand2 === 0) {
        operand2 = 1;
    }
